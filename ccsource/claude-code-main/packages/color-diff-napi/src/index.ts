...
function highlightLine(
  ...
  if (!hasRootNode((result as any).emitter)) {
    if (!loggedEmitterShapeError) {
      loggedEmitterShapeError = true
      logError(
        new Error(
          `color-diff: hljs emitter shape mismatch (keys: ${Object.keys((result as any).emitter).join(',')}). Syntax highlighting disabled.`,
        ),
      )
    }
    return [[defaultStyle(theme), code]]
  ...
  const blocks: Block[] = []
  flattenHljs((result as any).emitter.rootNode, theme, undefined, blocks)
  return blocks
}
