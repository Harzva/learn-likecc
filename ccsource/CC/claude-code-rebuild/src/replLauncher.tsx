...
function emitStartupDiag(phase: string): void {
  if (process.env.CLAUDE_CODE_STARTUP_DIAG !== '1') return;
  try {
...
export async function launchRepl(root: Root, appProps: AppWrapperProps, replProps: REPLProps, renderAndRun: (root: Root, element: React.ReactNode) => Promise<void>): Promise<void> {
  emitStartupDiag('inside launchRepl before import App');
  const {
    App
  } = await import('./components/App.js');
  emitStartupDiag('inside launchRepl after import App');
  const {
    REPL
  } = await import('./screens/REPL.js');
  emitStartupDiag('inside launchRepl after import REPL');
  emitStartupDiag('inside launchRepl before renderAndRun');
  await renderAndRun(root, <App {...appProps}>
      <REPL {...replProps} />
    </App>);
  emitStartupDiag('inside launchRepl after renderAndRun');
}
