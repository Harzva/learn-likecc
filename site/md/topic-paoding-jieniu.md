# 庖丁解牛专题 - Claude Code Course

> **在线页面**: https://harzva.github.io/learn-likecc/topic-paoding-jieniu.html  
> **本文件**: `site/md/topic-paoding-jieniu.md`  
> **更新时间**: 2026-04-09

## 概要

这是新的 **庖丁解牛专题总页**。

它不再只等于某一张 Claude Code 结构图，而是把“拆开看系统”这件事升成一条专题轴。当前先收两条子线：

- `topic-cc-unpacked-zh.html`：**CC 庖丁解牛**
- `topic-superset-unpacked.html`：**Superset 解构**

## 目录（对照 HTML）

- **子专题入口**
  - 📦 CC 庖丁解牛
  - 🛰️ Superset 解构
- **为什么要把它升成专题**
- **推荐阅读顺序**

## 各节摘要

### 子专题入口

把现有结构导览页降成子专题，同时新增 `Superset 解构` 子专题页，让“拆结构”和“拆调度”放到同一专题下。`meta-agent` 这条思想线现在由更具体的 Superset 结构页来承载。

### 为什么要把它升成专题

现在要拆的已经不只是 Claude Code 自己，还包括 Superset 这类外层 orchestration shell。所以“庖丁解牛”应该变成方法论，而不是只绑定一个页面。

### 推荐阅读顺序

建议：

1. 先看 `CC 庖丁解牛`
2. 再看 `Superset 解构`
3. 最后回到 Like Code 判断哪些能力要内生化
