...
本轮视觉升级把页面改成更强的“研究工作台”表达：Hero 增加 `quest repo / baseline / experiment / paper output` 轨道和三块工作台资产摘要，让 DeepScientist 和 Autoresearch 的差异在首屏就能看出来。
...
补充：页面已加入两张 Mermaid 图（六层架构图 + research loop）。
