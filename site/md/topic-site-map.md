...
- 子专题会继续从这些 hub 往下长，比如：
  ...
  - Agent 技术热点 / RAG 技术热点
  - 仿真大专题 / Loop Lab
  - Agent Loop Simulator / Script Insight Simulator / Trace Prompt Simulator
...
这次还把 `仿真大专题 / Loop Lab → Agent Loop Simulator / Script Insight Simulator / Trace Prompt Simulator` 挂进了主图。这样新做的 Trace Prompt 仿真器不再只是一个静态目录，而是作为 Loop Lab 的第三种仿真方式被纳入站点结构。

同一条支线里，`codex-loop AI Terminal / LikeCode Workspace App / Connector Runtime / Daemon Design` 继续作为控制台、工作台和运行时解释层存在，和三种仿真器形成“可交互样本 + 机制说明”的关系。
...
- `Loop Lab`
  - 看 `Agent Loop Simulator / Script Insight Simulator / Trace Prompt Simulator`
  - 再看 `AI Terminal / Workspace App / Connector Runtime`
- `Design/UI`
