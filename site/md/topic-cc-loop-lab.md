...
- **推荐学习路线导航**
  ...
  - 接事件
- **学习仪表盘**
  - 使用 `localStorage` 记录三台仿真器的访问、交互和阅读进度
  - 显示总完成率、完成课程数、最近学习和推荐下一课
  - 提供三张课程进度卡片与本机记录重置按钮
- **先按问题选入口**
...
- 写入来源：三个仿真器页面共享 `site/simulator-course-system.js`，记录访问次数、点击交互、最大滚动阅读比例、最近更新时间和完成状态。
- 总入口：`topic-cc-loop-lab.html` 读取同一份状态，渲染总完成率、最近学习、推荐下一课和三张课程卡。
