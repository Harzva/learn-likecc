...
- **学习仪表盘**
  ...
  - 提供 Data Vault：下载 JSON、复制 JSON、复制学习日志 Markdown、导入 JSON 文件或粘贴内容
- **先按问题选入口**
...
- 数据迁移：Data Vault 会把完整本机状态导出为 `learn-likecc.loop-lab.export` JSON 包，包含原始状态、课程摘要和可粘贴到 GitHub Issue 的 Markdown 学习日志。
- 导入校验：导入时只接受 Loop Lab 识别的课程字段，并会清洗进度、断点、hash、笔记和学习状态，避免把未知结构直接写回本机存储。
- 总入口：`topic-cc-loop-lab.html` 读取同一份状态，渲染总完成率、最近学习、推荐下一课、三张课程卡、断点提示、状态徽标、笔记预览、学习状态筛选和数据迁移面板。
- GitHub Pages 兼容性：全部能力依赖静态 HTML、CSS、前端 JS、URL query/hash、FileReader、Blob 与浏览器 `localStorage`，不需要后端服务或额外构建。
