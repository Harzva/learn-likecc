 ...
 */
;(function () {
    var mount = document.getElementById('hermes-arch-treemap-mount')
    if (!mount) return

    var jsonUrl = mount.getAttribute('data-json') || 'data/hermes-arch-treemap.json'
    ...
    function render(payload) {
        ...
        mount.innerHTML =
            ...
            '<h3 class="cc-arch-treemap__title">Hermes 六层架构 Explorer（教学向 · 双层）</h3>' +
            '<p class="cc-arch-treemap__sub">大块 = 架构层，可<strong>单击下钻</strong>；叶子块 = 子组件，<strong>单击复制</strong>名称。顶部路径可返回上一级或全盘。</p>' +
            '</div>' +
            ...
            '<svg class="cc-arch-treemap__svg" aria-label="Hermes 六层架构占比图"></svg>' +
            '</div>' +
        ...
        function draw() {
            ...
            cell.append('title').text(function (d) {
                if (d.children) {
                    return d.data.name + ' — 合计 ' + d.value + ' 个模块 · 单击下钻'
                }
                return d.data.name + ' — ' + d.value + ' 个模块 · 单击复制名称'
            })
