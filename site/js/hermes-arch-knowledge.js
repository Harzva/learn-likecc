/**
 * Hermes Agent architecture knowledge graph.
 * Modes:
 * - contains: 六层架构 -> 关键组件归属
 * - cross: 跨组件交互关系
 * - hybrid: 两者叠加
 * Mount: <div id="hermes-arch-knowledge-mount" data-json="data/hermes-arch-knowledge.json">
 */
;(function () {
    var mount = document.getElementById('hermes-arch-knowledge-mount')
    if (!mount) return

    var jsonUrl = mount.getAttribute('data-json') || 'data/hermes-arch-knowledge.json'
    var D3_SRC = 'https://cdn.jsdelivr.net/npm/d3@7.9.0/dist/d3.min.js'
    var COLORS = {
        entry_shell: '#cf9218',
        control_plane: '#8b5cf6',
        tool_surface: '#ea6a28',
        memory_skills: '#3a67df',
        platform_timeline: '#26a269',
        env_backend: '#7a8ba5',
        support: '#55657d',
    }
    ...
    function render(payload) {
        var d3 = window.d3
        var modes = [
            { id: 'hybrid', label: '混合总览', desc: '同时看架构层归属与组件间交互' },
            { id: 'contains', label: '层内结构', desc: '看六层架构下各有哪些关键组件' },
            { id: 'cross', label: '块间交互', desc: '看跨层组件之间最值得一起读的联系' },
        ]
        ...
        mount.innerHTML =
            ...
            '<div>' +
            '<h3 class="cc-arch-knowledge__title">Hermes 架构关系图谱</h3>' +
            '<p class="cc-arch-knowledge__subtitle">用网络图补足 Treemap 看不到的“谁驱动谁、谁依赖谁”。</p>' +
            '</div>' +
            '<div class="cc-arch-knowledge__controls"></div>' +
            '</div>' +
            '<div class="cc-arch-knowledge__layout">' +
            '<div class="cc-arch-knowledge__canvas"><svg class="cc-arch-knowledge__svg" aria-label="Hermes 架构知识图谱"></svg></div>' +
        ...
        function renderDetail(node, adjacency, allNodes) {
            if (!node) {
                detailEl.innerHTML =
                    ...
                    '<p class="cc-arch-knowledge__desc">左边网络图负责显示结构和联系，右边这里负责解释这个组件是什么、和谁连得最紧，以及为什么值得一起读。</p>' +
                    '<p class="cc-arch-knowledge__empty">推荐先点 <strong>AIAgent</strong>、<strong>gateway/run.py</strong>、<strong>tools/registry.py</strong>，或者切到 <strong>块间交互</strong> 看跨层驱动关系。</p>'
                return
            }
            ...
            detailEl.innerHTML =
                ...
                esc(node.kind === 'category' ? 'Architecture Layer' : 'Component') +
                '</p>' +
                ...
                (node.kind === 'category'
                    ? '架构层 · 教学权重约 <strong>' + esc(node.size) + '</strong>'
                    : '关键组件 · 教学权重约 <strong>' +
                      esc(node.size) +
                      '</strong> · 所属层 <strong>' +
                      esc(allNodes[node.parent] ? allNodes[node.parent].label : node.cat) +
                      '</strong>') +
                '</p>' +
                '<p class="cc-arch-knowledge__desc">' +
                esc(node.description || node.hint || '图谱中的结构节点。') +
                '</p>' +
                (node.analysis
        ...
        function draw() {
            ...
            var height = window.innerWidth <= 768 ? 520 : 700
