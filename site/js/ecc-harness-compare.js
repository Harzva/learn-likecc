/**
 * Everything Claude Code — 跨 harness 对照模块
 * Mount: <div id="ecc-harness-compare-mount" data-json="data/ecc-harness-compare.json">
 * Reuses .du-compare__* CSS from style.css (design-ui-compare pattern)
 */
;(function () {
    var mount = document.getElementById('ecc-harness-compare-mount')
    if (!mount) return

    var jsonUrl = mount.getAttribute('data-json') || 'data/ecc-harness-compare.json'
    ...
    function render(data) {
        ...
        block.items.forEach(function (item) {
            var accent = esc(item.accent || '#6366f1')
            html +=
    ...
    fetch(jsonUrl)
        ...
        .catch(function () {
            mount.innerHTML =
                '<p class="du-compare__err">无法加载 harness 对照数据（请用 HTTP 打开本站）。</p>'
        })
})()
