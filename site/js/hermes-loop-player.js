/**
 * Hermes Agent 主循环步进播放器
 * 挂载点：<div id="hermes-loop-player" data-json="data/hermes-loop-steps.json">
 * 改编自 cc-loop-player.js，保持交互契约一致。
 */
;(function () {
    var root = document.getElementById('hermes-loop-player')
    if (!root) return

    var jsonUrl = root.getAttribute('data-json') || 'data/hermes-loop-steps.json'
    var steps = []
    ...
    function buildChrome() {
        root.innerHTML =
            ...
            '</div>' +
            '<div class="cc-loop-player__strip" role="tablist" aria-label="Hermes Agent 主循环步骤"></div>' +
    ...
    fetch(jsonUrl)
        ...
        .catch(function () {
            fail('无法加载步进数据（请用 HTTP 打开本站，或检查 data/hermes-loop-steps.json）')
        })
})()
