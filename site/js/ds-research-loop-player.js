/**
 * DeepScientist 研究循环步进播放器
 * 挂载点：<div id="ds-research-loop-player" data-json="data/ds-research-loop.json">
 * 改编自 hermes-loop-player.js / cc-loop-player.js，保持交互契约一致。
 */
;(function () {
    var root = document.getElementById('ds-research-loop-player')
    if (!root) return

    var jsonUrl = root.getAttribute('data-json') || 'data/ds-research-loop.json'
    var steps = []
    ...
    function buildChrome() {
        root.innerHTML =
            ...
            '</div>' +
            '<div class="cc-loop-player__strip" role="tablist" aria-label="DeepScientist 研究循环步骤"></div>' +
