 ...
 */
;(function () {
    var mount = document.getElementById('ecc-knowledge-mount')
    if (!mount) return

    var jsonUrl = mount.getAttribute('data-json') || 'data/ecc-knowledge.json'
    ...
    function loadScript(src, cb) {
        ...
        s.onload = cb
        s.onerror = function () {
            mount.innerHTML = '<p class="ecc-knowledge__err">无法加载 D3，知识图谱未能渲染。</p>'
        }
    ...
    function render(payload) {
        ...
        mount.innerHTML =
            '<div class="ecc-knowledge__toolbar">' +
            '<div>' +
            '<h3 class="ecc-knowledge__title">Everything Claude Code 知识图谱</h3>' +
            '<p class="ecc-knowledge__subtitle">跨 harness 壳层系统的组件关系与五层结构映射。</p>' +
            '</div>' +
            '<div class="ecc-knowledge__controls"></div>' +
            '</div>' +
            '<div class="ecc-knowledge__layout">' +
            '<div class="ecc-knowledge__canvas"><svg class="ecc-knowledge__svg" aria-label="Everything Claude Code 知识图谱"></svg></div>' +
            '<aside class="ecc-knowledge__panel">' +
            '<div class="ecc-knowledge__legend"></div>' +
            '<div class="ecc-knowledge__detail"></div>' +
            '<div class="ecc-knowledge__note"></div>' +
            '</aside>' +
            '</div>'

        var controlsEl = mount.querySelector('.ecc-knowledge__controls')
        var svgEl = mount.querySelector('.ecc-knowledge__svg')
        var legendEl = mount.querySelector('.ecc-knowledge__legend')
        var detailEl = mount.querySelector('.ecc-knowledge__detail')
        var noteEl = mount.querySelector('.ecc-knowledge__note')
        var canvasEl = mount.querySelector('.ecc-knowledge__canvas')

        modes.forEach(function (mode) {
            var btn = document.createElement('button')
            btn.type = 'button'
            btn.className = 'ecc-knowledge__btn' + (mode.id === currentMode ? ' is-active' : '')
            btn.textContent = mode.label
            btn.title = mode.desc
            btn.addEventListener('click', function () {
                currentMode = mode.id
                selectedId = null
                Array.prototype.forEach.call(controlsEl.querySelectorAll('.ecc-knowledge__btn'), function (node) {
        ...
        ;(payload.legend || []).forEach(function (item) {
            ...
            pill.className = 'ecc-knowledge__legend-item'
            pill.innerHTML =
                '<span class="ecc-knowledge__legend-dot" style="background:' +
        ...
        function getModeLinks() {
            ...
            if (currentMode === 'contains') return allEdges.filter(function(e) { return e.kind === 'contains' })
            if (currentMode === 'cross') return allEdges.filter(function(e) { return e.kind === 'cross' })
            return allEdges
        }
        ...
        function renderDetail(node, adjacency, allNodes) {
            if (!node) {
                detailEl.innerHTML =
                    '<p class="ecc-knowledge__eyebrow">How To Read</p>' +
                    '<h4 class="ecc-knowledge__name">先点一个节点</h4>' +
                    '<p class="ecc-knowledge__desc">左边网络图负责显示结构和联系，右边这里负责解释这个块是什么、和谁连得最紧，以及为什么值得一起读。</p>' +
                    '<p class="ecc-knowledge__empty">推荐先点 <strong>目标 Harness</strong>、<strong>Agents</strong>、<strong>Skills</strong>，或者切到 <strong>块间联系</strong> 看跨 harness 分发关系。</p>'
                return
            }
            ...
            var relatedExplained = related
                ...
                .map(function (item) {
                    return (
                        '<li class="ecc-knowledge__bullet-item"><strong>' +
                        esc(item.label) +
                        '</strong>：' +
                        esc(item.note) +
                        '</li>'
            ...
            detailEl.innerHTML =
                '<p class="ecc-knowledge__eyebrow">' +
                esc(node.kind === 'category' ? 'Category' : 'Component') +
                '</p>' +
                '<h4 class="ecc-knowledge__name">' +
                esc(shortLabel(node.label)) +
                '</h4>' +
                '<p class="ecc-knowledge__meta">' +
                (node.kind === 'category'
                    ? '核心类别 · 包含 <strong>' +
                      ((payload.edges || []).filter(function(e) {
                          var s = typeof e.source === 'object' ? e.source.id : e.source
                          return s === node.id && e.kind === 'contains'
                      }).length) +
                      '</strong> 个子节点'
                    : '所属分区 <strong>' +
                      esc((allNodes[node.parent] ? allNodes[node.parent].label : node.cat) || '未分类') +
                      '</strong>') +
                '</p>' +
                '<p class="ecc-knowledge__desc">' +
                esc(node.description || node.hint || '这是图谱中的一个结构节点。') +
                '</p>' +
                (node.analysis
                    ? '<p class="ecc-knowledge__why-title">为什么重要</p>' +
                      '<p class="ecc-knowledge__desc">' +
                      esc(node.analysis) +
                      '</p>'
                    : '') +
                (node.read_hint
                    ? '<p class="ecc-knowledge__why-title">建议先看</p>' +
                      '<p class="ecc-knowledge__desc">' +
                      esc(node.read_hint) +
                      '</p>'
                    : '') +
                '<p class="ecc-knowledge__related">相关节点：</p>' +
                '<div class="ecc-knowledge__related-list">' +
                (related.length
                    ? related
                          .slice(0, 10)
                          .map(function (item) {
                              return (
                                  '<span class="ecc-knowledge__chip">' +
                                  esc(item.label) +
                          ...
                          .join('')
                    : '<span class="ecc-knowledge__empty">当前模式下没有额外连接。</span>') +
                '</div>' +
                (relatedExplained
                    ? '<div class="ecc-knowledge__why-block">' +
                      '<p class="ecc-knowledge__why-title">为什么连到这些块</p>' +
                      '<ul class="ecc-knowledge__bullet-list">' +
                      relatedExplained +
                      '</ul>' +
                      '</div>'
                    : '')
        ...
        function draw() {
            ...
            var link = linkLayer
                ...
                .attr('class', function (d) {
                    return 'ecc-knowledge__link ecc-knowledge__link--' + d.kind
                })
                ...
                .attr('stroke', function (d) {
                    if (d.kind === 'cross') {
                        ...
                        var base = COLORS[targetCat] || '#94a3b8'
                        return colorWithAlpha(d3, base, 0.55)
                    }
                    return null
                })
            ...
            var node = nodeLayer
                ...
                .data(nodes)
                .join('g')
                .attr('class', 'ecc-knowledge__node')
            ...
            node.append('title').text(function (d) {
                return (shortLabel(d.label) + ' · 权重 ' + d.size)
            })
    ...
    loadScript(D3_SRC, function () {
        fetch(jsonUrl)
            ...
            .then(render)
            .catch(function () {
                mount.innerHTML = '<p class="ecc-knowledge__err">无法加载知识图谱数据。</p>'
            })
    })
})()
