 ...
 */
;(function () {
    var mount = document.getElementById('ds-arch-knowledge-mount')
    if (!mount) return

    var jsonUrl = mount.getAttribute('data-json') || 'data/ds-arch-knowledge.json'
    ...
    function loadScript(src, cb) {
        ...
        s.onload = cb
        s.onerror = function () {
            mount.innerHTML = '<p class="ds-arch-knowledge__err">无法加载 D3，知识图谱未能渲染。</p>'
        }
    ...
    function render(payload) {
        ...
        mount.innerHTML =
            '<div class="ds-arch-knowledge__toolbar">' +
            '<div>' +
            '<h3 class="ds-arch-knowledge__title">DeepScientist Knowledge Graph</h3>' +
            '<p class="ds-arch-knowledge__subtitle">用网络图补足 Treemap 看不到的“谁和谁经常一起读”。</p>' +
            '</div>' +
            '<div class="ds-arch-knowledge__controls"></div>' +
            '</div>' +
            '<div class="ds-arch-knowledge__layout">' +
            '<div class="ds-arch-knowledge__canvas"><svg class="ds-arch-knowledge__svg" aria-label="Claude Code 架构知识图谱"></svg></div>' +
            '<aside class="ds-arch-knowledge__panel">' +
            '<div class="ds-arch-knowledge__legend"></div>' +
            '<div class="ds-arch-knowledge__detail"></div>' +
            '<div class="ds-arch-knowledge__note"></div>' +
            '</aside>' +
            '</div>'

        var controlsEl = mount.querySelector('.ds-arch-knowledge__controls')
        var svgEl = mount.querySelector('.ds-arch-knowledge__svg')
        var legendEl = mount.querySelector('.ds-arch-knowledge__legend')
        var detailEl = mount.querySelector('.ds-arch-knowledge__detail')
        var noteEl = mount.querySelector('.ds-arch-knowledge__note')
        var canvasEl = mount.querySelector('.ds-arch-knowledge__canvas')

        modes.forEach(function (mode) {
            var btn = document.createElement('button')
            btn.type = 'button'
            btn.className = 'ds-arch-knowledge__btn' + (mode.id === currentMode ? ' is-active' : '')
            btn.textContent = mode.label
            btn.title = mode.desc
            btn.addEventListener('click', function () {
                currentMode = mode.id
                selectedId = null
                Array.prototype.forEach.call(controlsEl.querySelectorAll('.ds-arch-knowledge__btn'), function (node) {
        ...
        ;(payload.legend || []).forEach(function (item) {
            ...
            pill.className = 'ds-arch-knowledge__legend-item'
            pill.innerHTML =
                '<span class="ds-arch-knowledge__legend-dot" style="background:' +
        ...
        function renderDetail(node, adjacency, allNodes) {
            if (!node) {
                detailEl.innerHTML =
                    '<p class="ds-arch-knowledge__eyebrow">How To Read</p>' +
                    '<h4 class="ds-arch-knowledge__name">先点一个节点</h4>' +
                    '<p class="ds-arch-knowledge__desc">左边网络图负责显示结构和联系，右边这里负责解释这个块是什么、和谁连得最紧，以及为什么值得一起读。</p>' +
                    '<p class="ds-arch-knowledge__empty">推荐先点 <strong>services</strong>、<strong>components</strong>、<strong>commands</strong>，或者切到 <strong>主线映射</strong> 看主循环每一步主要落在哪些目录块上。</p>'
                return
            }
            ...
            var relatedExplained = related
                ...
                .map(function (item) {
                    return (
                        '<li class="ds-arch-knowledge__bullet-item"><strong>' +
                        esc(item.label) +
                        '</strong>：' +
                        esc(item.note) +
                        '</li>'
            ...
            detailEl.innerHTML =
                '<p class="ds-arch-knowledge__eyebrow">' +
                esc(node.kind === 'category' ? 'Category' : 'Component') +
                '</p>' +
                '<h4 class="ds-arch-knowledge__name">' +
                esc(shortLabel(node.label)) +
                '</h4>' +
                '<p class="ds-arch-knowledge__meta">' +
                (node.kind === 'loop'
                    ...
                    : '文件量约 <strong>' +
                      ...
                      '</strong>') +
                '</p>' +
                '<p class="ds-arch-knowledge__desc">' +
                esc(node.description || node.hint || '这是图谱中的一个结构节点。') +
                '</p>' +
                (node.analysis
                    ? '<p class="ds-arch-knowledge__why-title">为什么重要</p>' +
                      '<p class="ds-arch-knowledge__desc">' +
                      esc(node.analysis) +
                      '</p>'
                    : '') +
                (node.read_hint
                    ? '<p class="ds-arch-knowledge__why-title">建议先看</p>' +
                      '<p class="ds-arch-knowledge__desc">' +
                      esc(node.read_hint) +
                      '</p>'
                    : '') +
                '<p class="ds-arch-knowledge__related">相关节点：</p>' +
                '<div class="ds-arch-knowledge__related-list">' +
                (related.length
                    ? related
                          .slice(0, 10)
                          .map(function (item) {
                              return (
                                  '<span class="ds-arch-knowledge__chip">' +
                                  esc(item.label) +
                          ...
                          .join('')
                    : '<span class="ds-arch-knowledge__empty">当前模式下没有额外连接。</span>') +
                '</div>' +
                (relatedExplained
                    ? '<div class="ds-arch-knowledge__why-block">' +
                      '<p class="ds-arch-knowledge__why-title">为什么连到这些块</p>' +
                      '<ul class="ds-arch-knowledge__bullet-list">' +
                      relatedExplained +
                      '</ul>' +
                      '</div>'
                    : '')
        ...
        function draw() {
            ...
            var link = linkLayer
                ...
                .attr('class', function (d) {
                    return 'ds-arch-knowledge__link ds-arch-knowledge__link--' + d.kind
                })
            ...
            var node = nodeLayer
                ...
                .data(nodes)
                .join('g')
                .attr('class', 'ds-arch-knowledge__node')
    ...
    loadScript(D3_SRC, function () {
        fetch(jsonUrl)
            ...
            .then(render)
            .catch(function () {
                mount.innerHTML = '<p class="ds-arch-knowledge__err">无法加载知识图谱数据。</p>'
            })
    })
})()
